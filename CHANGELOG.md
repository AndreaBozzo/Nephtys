...
- The batch middleware now drains its buffer before exiting, so events already accepted were no longer silently discarded when a stream is stopped or its pipeline is hot-swapped. It also reports a cancelled pipeline to the caller instead of accepting events a departed worker will never flush, and logs rather than ignores a failed batch marshal. (#37)
