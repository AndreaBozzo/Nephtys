...
- A shared connector conformance suite: one test per clause of the `StreamSource` contract, each run against every connector from a single table, with local fault fixtures behind it. (#31)
  ...
  **Adding a connector means adding a row to `connectorCases`.** There is no way to opt out of a clause, only to declare which of the two structural exceptions applies: a push connector has no upstream connection to lose, and a poller's session outlives any single poll. Faults come from in-process fixtures rather than mocks — one upstream per kind, each with the same three levers (deliver a well-formed event, deliver a malformed frame, go away mid-session) — so a disconnect means the right thing for each connector without the tests knowing the difference.
  ...
  `make soak` is the optional long run — sessions back to back for 30 seconds per connector, roughly a thousand each, ending on the same goroutine accounting. It is skipped unless `NEPHTYS_SOAK` is set, so CI stays deterministic and bounded. `docs/LIFECYCLE.md` §9 documents the suite and what each test discriminates.
