...
### Added
- A shared connector conformance suite: one test per clause of the `StreamSource` contract, each run against every connector from a single table, with local fault fixtures behind it. (#31)

  Connector tests were strong individually and were five separate understandings of the same contract. `internal/connector/conformance_test.go` now states it once — `Open` performs no remote I/O, `ready()` fires exactly once per session, cancellation ends a session with `nil`, `Close` releases what `Open` acquired and is safe without a `Run`, a publish failure does not end a session, a slow publish backpressures rather than drops, no goroutine of the package outlives a cancelled session, and an upstream that goes away ends a pull session with a reason without the source reconnecting on its own.

  **Adding a connector means adding a row to `connectorCases`.** There is no way to opt out of a clause, only to declare which structural exceptions apply, and both separate the pull connectors from the push ones: a push connector has no upstream connection to lose (and a poller's session outlives any single poll), and it serves each client in its own goroutine, so it publishes concurrently rather than one event at a time. Backpressure is therefore asserted in the form each actually has - a pull connector reads one frame at a time, a push connector holds its client's response open until the publish returns - while "a slow pipeline drops nothing" is asserted for all five. Faults come from in-process fixtures rather than mocks — one upstream per kind, each with the same three levers (deliver a well-formed event, deliver a malformed frame, go away mid-session) — so a disconnect means the right thing for each connector without the tests knowing the difference.

  Goroutine accounting counts goroutines whose stack runs through `internal/connector`, against a baseline in the same test. That is narrower than a process-wide count on purpose: a whole-process count is dominated by idle HTTP connections and would be flaky rather than strict.

  `make soak` is the optional accumulation run — 300 sessions per connector back to back, about three seconds for the whole suite, ending on the same goroutine accounting. It is bounded by session count rather than by wall time: every session opens and closes a TCP connection, a closed socket holds its port in TIME_WAIT for minutes, and a duration-bounded run therefore exhausts the ephemeral port range instead of finding anything — which is what the first version did. It is skipped unless `NEPHTYS_SOAK` is set, so CI stays deterministic and bounded. `docs/LIFECYCLE.md` §9 documents the suite and what each test discriminates.
- [`docs/REPLAY.md`](docs/REPLAY.md), the replay and recovery recipe, with [`docs/replay/consumer.go`](docs/replay/consumer.go) as a runnable example consumer and an integration suite covering it. (#14)

  Replay needed no new API surface — every accepted event is already in JetStream, and where a consumer starts reading is a consumer-side decision. What was missing was the documented procedure: where a replay can start (oldest, a stream sequence, a relative or absolute time, newest, newest-per-subject), how to recover after a consumer outage from a durable consumer's acknowledgement floor, and how to backfill a new consumer from a point in time. Every command in it was run against a live instance.

  The example consumer is one file with no dependency beyond the NATS client, and does the one thing the `nats` CLI cannot: start a replay at an absolute instant, since `--since` and `--deliver` take a duration ago and "from 06:00 UTC, when the incident started" is not naturally one.

  Two things it documents are traps rather than features. The client **deletes** the JetStream consumer behind any subscription it created when that subscription is drained or unsubscribed — a durable one included — so the obvious `defer sub.Drain()` destroys the acknowledgement floor a durable exists to keep, and does it asynchronously, so a process that exits promptly wins the race and appears to work. And a *push* consumer streams ahead of acknowledgements: stop one mid-replay and a restarted consumer receives nothing until `AckWait` expires, which for 30 seconds looks exactly like data loss. Both the recipe and the example consumer therefore pull, and close the connection rather than draining it.

  `internal/server/replay_test.go` asserts the contract through the real admission and publish path: replay from a sequence, from an instant, a durable consumer resuming after its connection is lost, subject scoping across streams that share the JetStream stream, and events staying replayable after `DELETE /v1/streams/{id}` removes the stream that produced them.
- `.claude/skills/code-review/SKILL.md`, a repository skill for working through a Copilot review on a Nephtys PR: how to read the half of a review the comments API does not return (a suppressed finding on #72 was the sharpest comment in it and appears only in the review body), how to verify a finding against the source before acting on it, and how to reply to and resolve threads. Every command in it was run against this repository, and it carries the accuracy record so far — eight findings, all pointing at something real, two with inaccurate specifics.
...
### Fixed
- **A gRPC client sending a payload that is not JSON broke every publish on its stream.** The gRPC source passed `req.Payload` through as `json.RawMessage` unchecked. `json.RawMessage` is validated when the envelope is marshalled, which happens on the publish path in the broker — so one client sending non-JSON produced a marshalling error raised a long way from the connector that caused it, failed the publish, and took that client's stream down with it. It now wraps a non-JSON payload as a JSON string, which is what the WebSocket, SSE, webhook and REST-poller sources have always done. Valid JSON is unaffected and still passes through byte for byte. Found by the conformance suite above, which is the shape of defect it exists to find. (#31)
- The conformance suite itself required `Close` to be idempotent, by closing a source explicitly and then again on cleanup. `StreamSource` promises exactly one `Close` per `Open` that returned nil, so a connector written to that contract could have failed the suite. The harness closes once now; the two push connectors that document a repeatable `Close` are tested for it beside their own implementations. Raised by a Copilot review on the pull request. (#31)
- **compose no longer pins `container_name` on any service.** Container names are global to the Docker daemon rather than scoped to the compose project, so `nephtys_nats` here made this stack and the paper's benchmark stack mutually exclusive: `docker compose up` failed outright while the other was running. Compose derives names from the project now (`nephtys-nats-1`); nothing referenced the pinned names, since everything inside the stack already addresses services by service name over the compose network. Published host ports still collide, which is inherent to publishing fixed ports. (#53)
- CI now runs on every pull request, not only those whose base is `main`. A branch stacked on another feature branch got no CI at all until the branch under it merged — which is after the point where the result would have been useful.
- Every `make docker-*` target now fails with "Docker is not reachable. Start Docker Desktop (or the daemon) and try again." instead of the daemon's raw named-pipe or socket error, which names everything except the cause.
- The README said a persisted config the current validator rejects is "skipped with a warning" at startup. It is registered in `error` state carrying the reason, so it stays visible and removable — which is the documented behaviour everywhere else, `docs/LIFECYCLE.md` included.
- `docs/CONTRIBUTING.md` described a project that has moved on: it invited contributions of the SSE, webhook and gRPC connectors that have all shipped, and its development setup started the full observability stack to run the tests. It now documents the actual loop (`make nats-up`, `make run`, `make smoke`, `make all`), what CI enforces, and the rules a change has to satisfy — generic-first, additive API surface, `--config-check` coverage for new config fields, and a changelog entry.
